...
public abstract class AlgorithmTest {
     ...
     */
	private final static boolean INFOS = true; // 
    ...
    public static void initAll() throws IOException {
		...
		arcInspectors[0] = ArcInspectorFactory.getAllFilters().get(0); // First ArcInspector = Shortest
		arcInspectors[1] = ArcInspectorFactory.getAllFilters().get(2); // Third ArcInspector = Fastest
    	...
    	for (int index=0; index < nodes.length; index++) {
    		for (int index2=0; index2 < nodes.length; index2++) {
    			...
    			for (ArcInspector AI : arcInspectors) {
			    	...
			    	ShortestPathSolution Solution = createSolution1();
    ...
    public void testIsValidCheminInexistant() {
    	...
    	for (ArcInspector AI : arcInspectors) {
	    	data = new ShortestPathData(defaultGraph, debut, fin, AI);
	    	assertEquals(Status.INFEASIBLE,createSolution1().getStatus());
		}		        
    }
		...
		for (int index=0; index < nodes.length; index++) {
    		for (int index2=0; index2 < nodes.length; index2++) {
    			...
    			for (ArcInspector AI : arcInspectors) {
    		    	data = new ShortestPathData(defaultGraph, debut, fin, AI);
    		    	ShortestPathSolution Solution = createSolution1();
    		    	ShortestPathSolution Solution2 = createSolution2();
    		    	
    		    	//assertEquals(Solution2.getStatus(), Solution.getStatus());

    		    	if (Solution.isFeasible() && Solution2.isFeasible()) assertEquals(Solution2.getPath().getLength(), Solution.getPath().getLength(), precision);
    		    	if (Solution.isFeasible() && Solution2.isFeasible()) assertEquals(Solution2.getPath().getMinimumTravelTime(), Solution.getPath().getMinimumTravelTime(), precision);
    			}
    		}
    	}
	}
	...
	/**
	 * @return The ShortestPathSolution using First Algorithm. (Tested)
	 */
	protected abstract ShortestPathSolution createSolution1();
	
	/**
	 * @return The ShortestPathSolution using Second Algorithm. (Reference)
	 */
	protected abstract ShortestPathSolution createSolution2();
}
